...
from datasets.video_dataset import MIVideoDataset
...
sweep_config = {
    ...
    'parameters': {
        'architecture': {
            'values': ['r2plus1d_18'], #  [ 'r3d_18', 'mc3_18'] , # 'r2plus1d_18'] # 
        },
        'fold': {
            'values': [0, 1, 2, 3, 4],
        },
...
def run_n_epochs(config=None):
    ...
    with wandb.init(config=config) as run:
        ...
        folder_exp = f'/home/vishc2/tuannm/echo/vishc-echo/models/mi_video_5f/{config.architecture}__{config.epoch:03d}__{config.fold:03d}'
        train_set = MIVideoDataset(split='train', id_fold=config.fold, img_size=config.img_shape)
        val_set = MIVideoDataset(split='val', id_fold=config.fold, img_size=config.img_shape)
        test_set = MIVideoDataset(split='test', id_fold=config.fold, img_size=config.img_shape)
...
def train():
    # run_n_epochs()
    sweep_id = wandb.sweep(sweep_config, project="mivideo32f_8train", entity='mtuann')
    wandb.agent(sweep_id, run_n_epochs, count=5)
